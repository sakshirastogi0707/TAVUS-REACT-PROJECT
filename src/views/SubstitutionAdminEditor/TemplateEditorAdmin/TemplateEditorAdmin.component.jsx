...
import { TemplateWord } from "../../../components/template-transcript/TemplateWord/TemplateWord.component";
import shortUUID from "short-uuid";
import SubstitutionModal from "../../../components/substitution/SubstitutionModal/SubstitutionModal.component";
import { VariableWords } from "../../../components/template-transcript/VariableWord/VariableWord.component";
import Loader from "../../../components/$widgets/loader/loader";
import { GVSProvider } from "../../../components/substitution/context/GVS.context";
...
const TemplateTranscript = ({ isLoading }) => {
  ...
  const { template, substitutions, isSubsLoading, submitATemplate } =
    useSubstitutions();
  ...
  if (loading || isLoading || isSubsLoading)
    return (
      <Loader
        styles={{ position: "relative", paddingTop: "50%", top: "50%" }}
      />
    );
  if (!template) {
    return (
  ...
  return (
    <GVSProvider>
      <Grid container justifyContent="space-between">
        <Grid item>
          <h4> Templated Transcript</h4>
        </Grid>
        <Grid item>
          <Button onClick={submitATemplate} variant="contained">
            Process
          </Button>
        </Grid>
      </Grid>
