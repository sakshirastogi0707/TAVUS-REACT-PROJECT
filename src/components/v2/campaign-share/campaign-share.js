...
import './campaign-share.scss';
import { html } from "./campaign-share.html";
...
class CampaignShare extends Component {
    ...
    componentDidMount() {
        this.campaignsWithAsign(this.state.page, this.state.searchField, true)
    ...
    openModel = (isDialogOpen, name) => {
        if (name == 'assignUser') {
            if (!this.state.selectedTemplates.length > 0) {
                return toast.warning('Please select any template.')
            }
        }
        this.setState({
            [name]: isDialogOpen,
    ...
    openModelAssign = async (data) => {
        ...
        let assignedUsers = []
        this.state.campaignsWithUsers && this.state.campaignsWithUsers[data] && this.state.campaignsWithUsers[data].assign_users && this.state.campaignsWithUsers[data].assign_users.map((val) => {
            if (val.user_info) {
                assignedUsers.push({
    ...
    deleteUserFromTemp = () => {
        ...
        HTTP.delete(urls.unassign, { data }).then((result) => {
            if (result.data.status) {
                ...
                this.handleSearchUser('')
                this.campaignsWithAsign(1, this.state.searchField, false)
            } else {
                this.setState({
                    removeUser: false,
                    delete_temp_id: ''
    ...
    pagination = (e) => {
        ...
        this.campaignsWithAsign(e, this.state.searchField, true)
    }
    ...
    handleSearchUser = (val) => {
        ...
        HTTP.get(urls.getUsers + params).then((result) => {
            if (result.data.status) {
                ...
                let assignedUsers = []
                this.state.campaignsWithUsers && this.state.campaignsWithUsers[tempUsers] && this.state.campaignsWithUsers[tempUsers].assign_users && this.state.campaignsWithUsers[tempUsers].assign_users.map((val) => {
                    if (val.user_info) {
                        assignedUsers.push({
                            value: val.user_info.id,
    ...
    campaignsWithAsign = (page= 1, search, choice) => {
        this.setState({ isLoading: choice })
        let params = '?page=' + page + '&limit=6&search=' + search
        HTTP.get(urls.campaignsWithAsign + params).then((result) => {
            if (result.data.status) {
                if (result.data?.data?.rows) {
                    this.setState({
                        campaignsWithUsers: result.data.data.rows,
    ...
    assignTemplate = (users, template) => {
        ...
        HTTP.post(urls.assignTemplate, data).then((result) => {
            if (result.data.status) {
                this.campaignsWithAsign(this.state.page, this.state.searchField, false)
                this.handleSearchUser('')
    ...
    handleChange = (data) => {
        ...
        //console.log('data', this.state.role)
    };
