properties properties: [
    parameters([
            ...
            string(defaultValue: 'develop-v2', description: 'Branch to deploy', name: 'BRANCH'),
    ])
]
pipeline {
    agent any
    environment {
        ...
        ENV_TAG=sh(script: 'cut -d- -f2 <<< $JOB_NAME', returnStdout: true).trim()
    ...
    // Building Docker images
        stage('Building image') {
            steps{
                script {
                    sh '''#!/bin/bash
                       sed -i '/CMD/c CMD ["npm", "run", "serve:\'${ENV_TAG}'"]' Dockerfile
                       sed -i '/build:/c RUN npm run build:'${ENV_TAG}'' Dockerfile
                    '''
	...
	// Deploy Application by deployment yaml to kubernetes cluster hosted on EKS
	    stage('Deploy Application to K8S Cluster') {
            steps {
		        script {
		             sh '''#!/bin/bash
		                   sed -i "/rollingStartDate/c \\ \\ \\ \\ \\ \\ \\ \\ rollingStartDate: $(date)" deployment.yaml
		             '''
			         kubernetesDeploy(
